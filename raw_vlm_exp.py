...
def main(cfg):
    ...
    for question_ind in tqdm(range(len(questions_data))):
        ...
        success_weighted_sum += success_weighted
        logging.info(f"Weighted answer: {pred_token}")
        ...
        success_max_sum += success_max
        logging.info(f"Max relevancy answer: {pred_token}")
        ...
        logging.info(f"Current success rate (weighted): {success_weighted_sum / cnt_data}")
        logging.info(f"Current success rate (max): {success_max_sum / cnt_data}")
    # Save all data again
    with open(os.path.join(cfg.output_dir, "results.pkl"), "wb") as f:
        pickle.dump(results_all, f)
    logging.info(f"\n== All Summary")
    logging.info(f"Number of data collected: {cnt_data}")
    logging.info("Success rate (weighted): {}".format(success_weighted_sum / cnt_data))
    logging.info("Success rate (max): {}".format(success_max_sum / cnt_data))
