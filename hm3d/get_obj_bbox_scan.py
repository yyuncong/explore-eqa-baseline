...
class GridSampler(MultisensorySimulator):
    ...
    def scan_scene(self, all_ids, scene, id2cate, return_features = True):
        gt_seg_dir = DATA_PATH + "/hm3d_obj_bbox_merged_val"
        if not os.path.exists(gt_seg_dir):
            os.mkdir(gt_seg_dir)
        ...
        for object_id in all_ids:
            obj_bbox = {}
            object_points = points[points[:, -1] == object_id]
            if object_points.shape[0] == 0: continue
            xmn, xmx = object_points[:,0].min(), object_points[:,0].max()
            ymn, ymx = object_points[:,1].min(), object_points[:,1].max()
            zmn, zmx = object_points[:,2].min(), object_points[:,2].max()
            box = [[float(xmn), float(zmn), float(ymn)], [float(xmx), float(zmx), float(ymx)]]
            obj_bbox["id"] = str(object_id) 
            obj_bbox["class_name"] = id2cate[object_id]
            obj_bbox["bbox"] = box 
            obj_bboxes.append(obj_bbox)
        
        with open(f"{gt_seg_dir}/{scene}.json", "w") as f:
            json.dump(obj_bboxes, f, indent=4)

        return points

    def get_per_instance(self, i, room, obs, all_ids):
        instance_feature_dict = dict()
        gt_seg_dir = DATA_PATH + "/original_2d_gt_seg_new"
...
if __name__ == "__main__":
    ...
    for scene_id in tqdm(val_scene_ids[14:]):
        ...
        points = sampler.scan_scene(all_ids, scene, id2cate, return_features=True)
        sampler.close()
